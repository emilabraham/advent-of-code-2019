...
# Get the direction of the wire instruction
def getDirection(instruction):
    return instruction[0:1]

# Get the magnitude of the wire instruction
def getMagnitude(instruction):
    return int(instruction[1:])

# Return a list of pairs indicating the path the wire traveled
def path(wire):
    x = 0
    y = 0
    path = []
    for i in wire:
        ...
        x, y = draw(path, direction, magnitude, x, y)
    return path

# Append pairs to path based on direction and magnitude
def draw(path, direction, magnitude, x, y):
    if direction == "R":
        while magnitude > 0:
            x = x + 1
            path.append((x, y))
...
# Find the intersections of 2 paths
def intersections(list1, list2):
    set1 = set(list1)
    set2 = set(list2)
    return (set1 & set2)

# Manhattan distance between given coordinate and origin
def manhattanDistance(coord1):
    x,y = coord1
    return abs(x) + abs(y)

# Find the shortest Manhattan distance intersection to the origin
def closestIntersection(wireIntersections):
    closest = 0
    for x, y in wireIntersections:
        dis = manhattanDistance((x,y))
        if closest == 0:
            closest = dis
        if dis < closest:
            closest = dis
...
path1 = path(wire1)
path2 = path(wire2)
testPath1 = path(testWire1)
testPath2 = path(testWire2)
